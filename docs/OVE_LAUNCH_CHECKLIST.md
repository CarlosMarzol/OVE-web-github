...
- 2026-07-30: Migrated form delivery from Resend to Outlook SMTP via `api/contact.js`. Required deployment variables: `OVE_SMTP_USER`, `OVE_SMTP_PASS`, optional `OVE_SMTP_HOST`, `OVE_SMTP_PORT`, `OVE_SMTP_SECURE`, `OVE_CONTACT_TO`, and `OVE_CONTACT_FROM`. The homepage newsletter form now posts directly to `/api/contact` instead of opening a mailto link.
